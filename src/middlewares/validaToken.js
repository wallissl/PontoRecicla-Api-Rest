...
function validaToken(req, res, next) { // Cria a função validaToken. A função recebe os parâmetros req, res e next. O req é o objeto da requisição, o res é o objeto da resposta e o next é uma função que chama o próximo middleware na pilha de middlewares. O next é usado para passar o controle para o próximo middleware na pilha de middlewares. Se não houver mais middlewares na pilha, o next chama a função final que envia a resposta ao cliente.
    try{
        ...
        const resultado = verify(jwt[1], process.env.SECRET_JWT) // Verifica se o token é válido. O verify é usado para verificar se o token é válido. O método recebe o token e a chave secreta como parâmetros. Se o token for válido, o método retorna os dados do usuário. Se o token não for válido, o método lança um erro.
        ...
        console.log(error) // Imprime o erro no console. O erro é um objeto com os dados do erro. O objeto contém a mensagem de erro e o código de erro.
